            ...
            Confirm.show('溫馨提示', '您確定要刪除' + name + '嗎？', {
                'Delete': {
                    ...
                    'callback': function () {
                        ...
                        $.post("${pageContext.request.contextPath}/video/delete", params, function (data) {
                            if (data == 'success') {
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse"
            ...
            <p class="navbar-text navbar-right">
                <span>欢迎您!${userName}</span> <i class="glyphicon glyphicon-log-in"
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <form class="navbar-form navbar-right" action="${pageContext.request.contextPath}/video/list" method="post">
                ...
                <div class="btn-group">
                    <button type="button" id="speaker_name"
                            class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                            aria-haspopup="true" aria-expanded="false">
                        <c:forEach items="${speakerList}" var="speaker">
                            <c:if test="${speaker.id == queryVo.speaker_id}">
                                ${speaker.speaker_name}
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty queryVo.speaker_id}">
                            --请选择老师--
                        </c:if>
                    ...
                    <input type="hidden" name="speaker_id" id="speaker_id" value="${queryVo.speaker_id}"/>
                </div>
                <div class="btn-group">
                    <button type="button" id="course_name"
                            class="btn btn-primary dropdown-toggle" data-toggle="dropdown"
                            aria-haspopup="true" aria-expanded="false">
                        <c:forEach items="${courseList}" var="course">
                            <c:if test="${course.id == queryVo.course_id}">
                                ${course.course_title}
                            </c:if>
                        </c:forEach>
                        <c:if test="${empty queryVo.course_id}">
                            --请选择课程--
                        </c:if>
                        <span class="caret"></span>
                    </button>
                    ...
                    <input type="hidden" name="course_id" id="course_id" value="${queryVo.course_id}"/>
                </div>
        ...
        <table class="table table-bordered table-hover"
            ...
            <c:forEach items="${page.rows}" var="video" varStatus="status">
                <tr>
                    ...
                    <td>${video.course.course_title}</td>
                    <td>${video.time}</td>
